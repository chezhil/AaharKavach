...
def scan_url_endpoint(caller: Caller, body: dict[str, Any]) -> tuple[int, Any]:
    ...
    evaluation = evaluate(product, profiles, _alternatives(product, profiles, _deadline()))
...
#: How long one request may spend on the model before the rest of the work has
#: to make do without it. API Gateway cuts the connection at 29s and cannot be
#: raised, and a single evaluate makes two agent calls — Swap It for the
#: alternatives and then the verdict itself. When the provider is slow or rate
#: limited, the two together overrun and the caller gets a 504 rather than an
#: answer. The verdict is the product; alternatives are a nicety, so the
#: verdict gets the budget and Swap It is skipped once it is gone.
AGENT_BUDGET_SECONDS = float(os.environ.get("AAHAR_AGENT_BUDGET_SECONDS", "12"))


def _deadline() -> float:
    """A monotonic timestamp after which optional model calls are skipped."""
    return time.monotonic() + AGENT_BUDGET_SECONDS
...
def _alternatives(
    ...
    if os.environ.get("AAHAR_USE_AGENT", "").lower() == "true":
        if deadline is not None and time.monotonic() >= deadline:
            logger.info("Skipping Swap It: agent budget spent, using the catalogue")
            raise _BudgetSpent
        try:
...
def evaluate_endpoint(caller: Caller, body: dict[str, Any]) -> tuple[int, Any]:
    ...
    result = evaluate(product, profiles, _alternatives(product, profiles, _deadline()))
